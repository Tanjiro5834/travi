...
public class TripDayService {
   ...
   @Transactional
    public TripDayResponse createTripDay(TripDayRequest tripDayRequest) {
        Trip trip = tripRepository.findById(tripDayRequest.getTripId())
            .orElseThrow(() -> new RuntimeException(
                "Trip not found with id: " + tripDayRequest.getTripId()
            ));
    ...
    public List<TripDay> getByTripId(Long tripId) {
        return tripDayRepository.findByTripIdWithActivities(tripId);
    }

    public List<TripDayResponse> getDaysByTripId(Long tripId) {
        return tripDayRepository.findByTripIdWithActivities(tripId)
                .stream()
                .map(tripDayMapper::toResponse)
                .toList();
