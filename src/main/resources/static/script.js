/* =============================================
   WAYFARER — script.js
   API-driven: aligned to Spring Boot controllers
   ============================================= */
...
let packingState = JSON.parse(localStorage.getItem('wayfarer_packing') || '{}');
...
function setToken(token) {
  localStorage.setItem('wayfarer_token', token);
}
function clearToken() {
...
function showToast(msg, type = 'info') {
  let toast = document.getElementById('wayfarerToast');
  if (!toast) {
    toast = document.createElement('div');
    toast.id = 'wayfarerToast';
    toast.style.cssText = `
...
function initSearch() {
  ...
  input.addEventListener('input', () => {
    ...
    const results = countriesCache.filter(c =>
  ...
  input.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      const q = input.value.trim().toLowerCase();
      const match = countriesCache.find(c => (c.name || '').toLowerCase().includes(q));
      if (match) { openCountry(match.id); dropdown.classList.remove('visible'); input.value = ''; }
    }
  });
  ...
  document.addEventListener('click', e => {
    if (!e.target.closest('.hero-search-wrap')) dropdown.classList.remove('visible');
  });
}
...
function initHeroChips() {
  document.querySelectorAll('.tag-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      const match = countriesCache.find(c => (c.name || '').toLowerCase() === label);
...
async function openCountry(id) {
  ...
  try {
    ...
    const [country, requirements, budget, checklist, tips, culture] = await Promise.all([
    ...
    // Banner
    $('countryBanner').style.background = country.gradientCss || defaultGradient(0);
...
function populateBudget(b) {
  if (!b) {
    $('budgetLayout').innerHTML = '<div class="budget-tips-card"><div class="budget-tips-title">Budget data unavailable.</div></div>';
    return;
  }

  // Support pre-built tiers array from service
  let tiers = [];
  if (Array.isArray(b.tiers) && b.tiers.length) {
    tiers = b.tiers;
  } else {
    // Build from flat BudgetGuide fields
    tiers = [
      {
        name: 'Budget',
        perDay: formatRange(b.budgetTierMin, b.budgetTierMax),
        featured: false,
        items: buildBudgetItems(b, 'budget'),
      },
      {
        name: 'Mid-range',
        perDay: formatRange(b.midTierMin, b.midTierMax),
        featured: true,
        items: buildBudgetItems(b, 'mid'),
      },
      {
        name: 'Luxury',
        perDay: formatRange(b.luxuryTierMin, null),
        featured: false,
        items: buildBudgetItems(b, 'luxury'),
      },
    ].filter(t => t.perDay !== '—');
  }

  const tiersHTML = tiers.map(t => `
    <div class="budget-tier${t.featured ? ' featured' : ''}">
      ${t.featured ? '<div class="featured-badge">Most Popular</div>' : ''}
      <div class="budget-tier-name">${t.name}</div>
      <div class="budget-tier-price">${t.perDay || '—'}</div>
      <div class="budget-per-day">per person / day</div>
      <div class="budget-items">
        ${(t.items || []).map(i => `
          <div class="budget-item">
            <span class="budget-item-label">${i.label}</span>
            <span class="budget-item-value">${i.value}</span>
          </div>
        `).join('')}
      </div>
    </div>
  ...
  // moneyTips can be List<MoneyTip{icon,text}> or a JSON string
  let tips = [];
  if (Array.isArray(b.moneyTips)) tips = b.moneyTips;
  else if (typeof b.moneyTips === 'string') {
    try { tips = JSON.parse(b.moneyTips); } catch (_) {}
  }

  const tipsHTML = tips.map(t => `
    <div class="budget-tip">
      <span class="budget-tip-icon">${t.icon || '💡'}</span>
      <span class="budget-tip-text">${t.text}</span>
    </div>
  `).join('');

  $('budgetLayout').innerHTML = `
    <div class="budget-tier-grid">${tiersHTML || '<p style="padding:1rem">No budget tiers available.</p>'}</div>
    ${tips.length ? `<div class="budget-tips-card"><div class="budget-tips-title">💡 Money-Saving Tips</div>${tipsHTML}</div>` : ''}
  `;
}
...
function buildBudgetItems(b, tier) {
  ...
  return [
    { label: 'Accommodation', value: b[`${tier}Accommodation`] || b[`${cap(tier)}Accommodation`] },
    { label: 'Food', value: b[`${tier}Food`] || b[`${cap(tier)}Food`] },
    { label: 'Transport', value: b[`${tier}Transport`] || b[`${cap(tier)}Transport`] },
    { label: 'Activities', value: b[`${tier}Activities`] || b[`${cap(tier)}Activities`] },
  ].filter(i => i.value);
}
...
function populatePacking(categories) {
  ...
  if (!categories || !categories.length) {
    $('packingLayout').innerHTML = '<div class="info-card"><p class="entry-desc">Packing checklist unavailable.</p></div>';
    return;
  }
  ...
  const updateProgress = () => {
    const checked = Object.values(packingState[key] || {}).filter(Boolean).length;
  ...
  const catsHTML = categories.map((cat, ci) => `
  ...
  document.querySelectorAll('.pack-item').forEach(item => {
    item.addEventListener('click', () => {
      ...
      localStorage.setItem('wayfarer_packing', JSON.stringify(packingState));
      updateProgress();
    });
  });
}
...
function populateTips(tipsMap) {
  if (!tipsMap) {
    $('tipsLayout').innerHTML = '<div class="tips-grid"><p class="entry-desc">Local tips unavailable.</p></div>';
    return;
  }
  ...
  if (typeof tipsMap === 'object' && !Array.isArray(tipsMap)) {
    Object.entries(tipsMap).forEach(([category, list]) => {
      (list || []).forEach(t => allTips.push({ ...t, tag: t.category || category }));
    });
  } else if (Array.isArray(tipsMap)) {
    tipsMap.forEach(t => allTips.push({ ...t, tag: t.category || t.tag || '' }));
  }

  if (!allTips.length) {
    $('tipsLayout').innerHTML = '<div class="tips-grid"><p class="entry-desc">No tips available.</p></div>';
    return;
  }
...
function populateCulture(cultureMap) {
  if (!cultureMap) {
    $('cultureLayout').innerHTML = '<p class="entry-desc">Culture guide unavailable.</p>';
    return;
  }
  ...
  const entries = typeof cultureMap === 'object' && !Array.isArray(cultureMap) ? cultureMap : {};
  ...
  Object.entries(entries).forEach(([type, list]) => {
    ...
    (list || []).forEach(item => {
      ...
      else if (key === 'DONT' || key === 'DONTS' || key === "DON'T" || key === "DON'TS") donts.push(obj);
...
async function checkIfSaved(countryId) {
  ...
  try {
    ...
    return list.some(s => s.countryId === countryId || s.country?.id === countryId);
